...
/// WMG_Axis_Graph 차트에 일련의 데이터를 표시할 때 사용한다.
/// </summary>
public class WMG_Series : MonoBehaviour {
	...
	/// WMG_Axis_Graph::GraphType = 콤보일 때, 이 시리즈가 선으로 표시되는지 막대로 표시되는지 여부를 결정한다.
	/// </summary>
	...
	/// WMG_Axis_Graph::axesType = dual_y가 되면 이 시리즈가 두 번째 y축을 사용할지 여부가 결정된다.
	/// </summary>
	...
	public bool useSecondYaxis { get {return _useSecondYaxis;} 
		set {
			if (_useSecondYaxis != value) {
				if (!theGraph.IsDualY && value == true) {
	...
	/// 그래프 범례 또는 툴팁에 표시될 수 있는 이 시리즈의 이름.
	/// </summary>
	...
	/// #usePointColors = true가 아닌 한, 이 시리즈의 모든 포인트에 적용된 색상.
	/// </summary>
	...
	/// 활성화된 경우, #pointColors를 사용하여 각 점 색상을 개별적으로 할당할 수 있다.
	/// </summary>
	...
	/// 라인 시리즈에 대해 이 시리즈의 모든 라인에 적용된 색상.
	/// </summary>
	...
	/// x 값이 #pointValues에 대해 어떤 데이터인지에 관계없이 x축을 따라 시계열 데이터를 균등하게 배치하려면 이 옵션을 활성화하십시오.
	/// </summary>
	...
	/// #xDistBetweenPoints를 수동으로 제어하려면 이 옵션을 활성화하십시오.
	/// </summary>
	...
	/// #UseXDistBetweenToSpace = true일 때 각 영상 시리즈 지점 사이의 x 거리는 #ManualSetXDistBetween = true가 아닌 한 자동으로 제어된다.
	/// </summary>
	...
	/// 활성화되면 #extraXSpace를 수동으로 설정할 수 있으며 그렇지 않으면 자동으로 설정된다.
	/// </summary>
	...
	/// 축에서 이 시계열 점의 간격(예: 측면 막대 차트의 경우, 여기서 후기 시계열은 더 높은 값을 갖는다).
	/// </summary>
	...
	/// 활성화된 경우 이 모든 영상 시리즈 데이터 점을 숨기며, 선형 차트에 대한 선만 표시하는 데 유용하다.
	/// </summary>
	...
	/// 활성화된 경우 이 모든 영상 시리즈 선을 숨기십시오. 산점도에 대한 점만 표시하는 데 유용함.
	/// </summary>
	...
	/// 삼각형/원 같은 도형을 만드는 데 유용한 첫 번째 점과 마지막 점 사이에 선을 연결한다.
	/// </summary>
	...
	/// 라인 시리즈의 경우, 각 개별 라인 세그먼트에 대해 끝에 패딩된 여분의 공간의 양을 제어하며, 더 작은 라인 세그먼트를 생성하기 위해 음수일 수도 있다.
	/// </summary>
	...
	/// 점/막대 위에 나타나 해당 점/막대가 나타내는 값을 표시할 수 있는 텍스트 개체인 데이터 레이블을 표시할지 여부.
	/// </summary>
	...
	/// #dataLabelsEnabled = true이면 표시되는 소수점 수를 제어한다.
	/// </summary>
	...
	/// #dataLabelsEnabled = true이면 텍스트 레이블의 글꼴 크기를 제어한다.
	/// </summary>
	...
	/// #dataLabelsEnabled = true이면 텍스트 레이블의 색상을 제어한다.
	/// </summary>
	...
	/// #dataLabelsEnabled = true이면 텍스트 레이블의 글꼴 스타일을 제어한다.
	/// </summary>
	...
	/// #dataLabelsEnabled = true이면 텍스트 레이블의 글꼴을 제어한다.
	/// </summary>
	/// <value>The data labels font.</value>
#if TMP_PRESENT
	public TMP_FontAsset dataLabelsFont { get {return _dataLabelsFont;} 
		set {
	...
	/// #dataLabelsEnabled = true이면 데이터 레이블이 점/바의 왼쪽/아래에 고정되어 있는지 여부를 제어한다.
	/// Useful to overlay data labels on top of bars for bar charts.
	/// 막대형 차트의 막대 맨 위에 데이터 레이블을 오버레이하는 데 유용하다.
	/// </summary>
	...
	/// #dataLabelsEnabled = true이면 위치상 데이터 레이블을 상쇄한다.
	/// </summary>
	...
	/// 이 시리즈에 영역 음영이 있는지 여부와 그라데이션 또는 솔리드 색상인지 여부를 제어한다.
	/// </summary>
	...
	/// <summary>
	/// When enabled, controls whether or not a compute shader is used to draw lines.
	/// 활성화된 경우 컴퓨터 셰이더가 선을 그리는 데 사용되는지 여부를 제어한다.
	/// Compute shader should be used for high performance, unless the platform to which you are publishing does not support Compute shaders (refer to Unity documentation on Compute Shader).
	/// 게시하는 플랫폼이 컴퓨팅 셰이더를 지원하지 않는 한 고성능을 위해 컴퓨팅 셰이더를 사용해야 한다(컴퓨팅 셰이더의 Unity 설명서 참조).
	/// </summary>
	/// <value><c>true</c> if area shading uses compute shader; otherwise, <c>false</c>.</value>
	public bool linesUseComputeShader { get {return _linesUseComputeShader;} 
		set {
			if (_linesUseComputeShader != value) {
				_linesUseComputeShader = value;
				areaShadingTypeC.Changed();
			}
		}
	}
	...
	/// #areaShadingType != None이면 영역 음영을 계산하는 데 컴퓨팅 셰이더를 사용할지 여부를 제어한다.
	...
	/// 게시하는 플랫폼이 컴퓨팅 셰이더를 지원하지 않는 한, 컴퓨팅 셰이더를 항상 사용해야 한다(컴퓨팅 셰이더의 Unity 설명서 참조).
	/// </summary>
	/// <value><c>true</c> if area shading uses compute shader; otherwise, <c>false</c>.</value>
	...
	/// #areaShadingType !=None일 경우 영역 음영 색상을 제어한다.
	/// </summary>
	...
	/// #areaShadingType != None일 경우 영역 음영의 끝 y축 값을 제어한다.
	/// </summary>
	...
	/// 라인 시리즈에 대한 포인트 생성에 사용되는 프리팹을 지정하며, WMG_Axis_Graph::pointPrefabs의 지수에 해당한다.
	/// </summary>
	...
	/// 라인 시리즈에 대한 라인 생성에 사용되는 프리팹을 지정하며, WMG_Axis_Graph::linkPrefabs의 지수에 해당한다.
	/// </summary>
	...
	/// #areaShadingType == Solid 및 #areaShadingUsComputeShader = false일 때 영역 음영에 사용되는 재료.
	/// </summary>
	...
	/// #areaShadingType == Gradient 및 #areaShadingUsComputeShader = false일 때 영역 음영에 사용되는 재료.
	/// </summary>
	public Material areaShadingMatGradient;
	/// <summary>
	/// The parent GameObject for area shading rectangle(s).
	/// 영역 음영 사각형에 대한 상위 GameObject.
	/// </summary>
	public GameObject areaShadingParent;
	/// <summary>
	/// The area shading prefab when #areaShadingUsesComputeShader = false.
	/// #areaShadingUsComputeShader = false일 때 영역 음영 사전 프리팹.
	/// </summary>
	public UnityEngine.Object areaShadingPrefab;
	/// <summary>
	/// The area shading prefab when #areaShadingUsesComputeShader = true.
	/// #areaShadingUsComputeShader = true일 때 영역 음영 사전 프리팹.
	/// </summary>
	public UnityEngine.Object areaShadingCSPrefab;
	/// <summary>
	/// When enabled, never creates any line objects for this series, useful to for example create a points only scatter plot that will always be only points.
	/// 활성화된 경우 이 시리즈에 대한 선 객체를 만들지 마십시오. 예를 들어 항상 점만 되는 점만 산점도를 만드는 데 유용함.
	/// </summary>
	public bool neverCreateLines;
	/// <summary>
	/// When enabled, never creates any point objects for this series, useful to for example create a lines only graph.
	/// 활성화된 경우 이 영상 시리즈에 대한 점 객체를 만들지 마십시오. 예를 들어 선만 그래프를 만드는 데 유용함.
	/// </summary>
	public bool neverCreatePoints;
	/// <summary>
	/// When enabled, this series represents multiple series. This is intended to be used only for drawing massive number of series and data using the texture based drawing
	/// 활성화된 경우 이 영상 시리즈는 다중 영상 시리즈를 나타낸다. 이는 텍스처 기반 도면을 사용하여 방대한 수의 영상 시리즈와 데이터를 그리는 데만 사용하고자 함
	/// </summary>
	public bool multiSeriesEnabled;
	/// <summary>
	/// The number of series to be drawn
	/// 그릴 영상 시리즈 수
	/// </summary>
	public int multiSeriesNumSeries = 1;
	...
	/// #areaShadingType != 없음 및 #areaShadingUsComputeShader = true이면 전체 영역 음영 사각형을 생성하는 데 사용되는 텍스처의 분해능이다.
	/// </summary>
	...
	#if TMP_PRESENT
	[SerializeField] private TMP_FontAsset _dataLabelsFont;
	#else
	...
	[SerializeField] private bool _linesUseComputeShader;
	...
	public WMG_Axis yAxis { 
		get {
			if (theGraph.IsDualY && useSecondYaxis && theGraph.yAxis2 != null) {
	...
	private int numPointsPerSeries { 
		get {
			return (multiSeriesEnabled && multiSeriesNumSeries > 0) ? (pointValues.Count / multiSeriesNumSeries) : pointValues.Count;
		}
	}
	...
	private List<Color> multiSeriesTextureLineColors;

	// Original property values for use with dynamic resizing
	// 동적 크기 조정과 함께 사용할 원래 속성 값
	...
	private WMG_Change_Obj pointValuesC = new WMG_Change_Obj();
	private WMG_Change_Obj pointValuesCountC = new WMG_Change_Obj();
	...
	/// 데이터 레이블의 기본 레이블을 재정의하려면 이 항목을 사용하십시오(#dataLabelsEnabled = true일 때 점 위에 표시됨).
	/// @code
	...
	public string formatSeriesDataLabel(WMG_Series series, float val, int labelIndex) {
		Vector2 pointVal = series.pointValues [labelIndex];
		if (series.theGraph.useGroups && pointVal.x < 0)
		{ // graph uses groups and series x point is negative always means NULL 그래프에서 그룹을 사용하며 시계열 x 점이 음수인 경우 항상 NULL을 나타냄
			return "";
		}
	...
	/// <summary>
	/// Whether or not the series allows tooltip, still respecting the graph-level tooltip setting.
	/// 그래프 수준 툴팁 설정을 계속 유지하면서 시리즈가 툴팁을 허용하는지 여부.
	/// </summary>
	public bool seriesTooltipEnabled = true;
	...
	/// 기본 툴팁 영상 시리즈 지점 호버 애니메이션을 재정의하는 데 사용하십시오.
	/// @code
	...
	/// 이 시리즈가 자동으로 애니메이션화되기 시작할 때 발생(자동 애니메이션을 활성화해야 함).
	/// @code
	...
	/// GameObject 포인트가 인스턴스화된 후 발생. 사용자 지정 스크립트를 생성할 때 각 점에 동적으로 추가하는 데 유용함.
	/// @code
	...
	public delegate void LineCreatedHandler(WMG_Series series, GameObject line, int lineIndex);
	/// <summary>
	/// Occurs after a line GameObject is instantiated. Useful to dynamically add custom script to each line as it is created.
	/// 선 GameObject가 인스턴스화된 후 발생. 사용자 지정 스크립트를 생성할 때 각 행에 동적으로 추가하는 데 유용함.
	/// @code
	/// series.LineCreated += MyLineCreatedFunc;
	/// void MyLineCreatedFunc(WMG_Series series, GameObject point, int pointIndex) {}
	/// @endcode
	/// </summary>
	public event LineCreatedHandler LineCreated;
	protected virtual void OnLineCreated(WMG_Series series, GameObject line, int lineIndex) {
		LineCreatedHandler handler = LineCreated;
		if (handler != null) {
			handler(series, line, lineIndex);
		}
	}
	...
	/// 포인트 GameObject 비주얼 변경 / 위치 조정 후 발생. 치수나 위치와 같은 점 외관을 동적으로 변경하는 데 유용하다. 촛대 차트와 같은 고급 차트를 만드는 데 사용할 수 있다.
	/// @code
	...
	/// WMG_Axis_Graph::addSeries에서 자동으로 호출되는 이 시리즈를 초기화하십시오.
	/// </summary>
	...
	/// 그래프 새로 고침 프로세스 중에 사용되며, 그래프 작성기 코드 외부로 호출해서는 안 된다.
	/// </summary>
	...
	// Utility function to take into account stacked graph vs. individual series change event
	// 누적 그래프 대 개별 시리즈 변경 이벤트를 고려하는 유틸리티 기능
	public void pointValuesCountCChanged() {
		if (theGraph.IsStacked) {
			theGraph.stackedSeriesCountC.Changed();
		}
		else {
	...
	// Utility function to take into account stacked graph vs. individual series change event
	// 누적 그래프 대 개별 시리즈 변경 이벤트를 고려하는 유틸리티 기능
	void pointValuesValCChanged() {
		if (theGraph.IsStacked) {
			theGraph.stackedSeriesNoCountC.Changed();
		}
		else {
	...
	// Utility function to take into account stacked graph vs. individual series change event
	// 누적 그래프 대 개별 시리즈 변경 이벤트를 고려하는 유틸리티 기능
	public void pointValuesCChanged () {
	...
	public void pointValuesCountChanged() {
		pointValuesCountChangedSeries (false);
	}

	public void pointValuesCountChangedSeries(bool updatePrefab) {
		if (updatePrefab) {
			UpdatePrefabType();
		}
	...
	void PointValuesValChanged() {
		if (changedValIndices.Count != 1) { // multiple single vals changed in a single frame	단일 프레임에서 여러 개의 단일 벨류가 변경됨
			theGraph.aSeriesPointsChanged ();
			pointValuesChanged();
		}
		else {
	...
	void AreaShadingChanged() {
		if (areaShadingUsesComputeShader || linesUseComputeShader) {
			UpdateSprites();
		}
		else {
			updateAreaShading();
	...
	public void setAnimatingFromPreviousData() {
		// Automatic animations doesn't work for real time updating or stacked graphs	실시간 업데이트 또는 누적 그래프에 대해 자동 애니메이션이 작동하지 않음
	...
	/// #Init 동안 자동으로 호출되는 비율 기반 동적 크기 조정과 함께 사용할 초기 속성 값을 설정하십시오.
	/// </summary>
	...
	/// <summary>
	/// Sets a data point at the specified index (0 = first element). If the index goes beyond the size of the list, then add (0,0) elements to the list until the index element exists.
	/// 지정된 색인(0 = 첫 번째 요소)에서 데이터 점을 설정한다. 인덱스가 목록 크기를 초과하면 인덱스 요소가 존재할 때까지 목록에 요소를 추가(0,0)하십시오.
	/// </summary>
	/// <param name="index">Index.</param>
	/// <param name="val">Value.</param>
	public void setDatapoint(int index, Vector2 val) {
		if (index >= pointValues.Count) {
			int elementsToAdd = index + 1 - pointValues.Count;
			for (int i = 0; i < elementsToAdd; i++) {
				pointValues.Add(Vector2.zero);
				
			}
		}
		pointValues[index] = val;
	}

	/// <summary>
	/// Removes the datapoint at the specified index (0 = first element).
	/// 지정된 색인(0 = 첫 번째 요소)에서 기준점을 제거한다.
	/// </summary>
	/// <param name="index">Index.</param>
	public void removeDatapoint(int index) {
		pointValues.RemoveAt(index);
	}
	...
	void UpdateNullVisibility(int index = -1) {
		// For null groups hide the appropriate points	null 그룹의 경우 적절한 점을 숨기기
		if (theGraph.useGroups) {
			if (!neverCreatePoints) {
				if (index < 0) {
					int pointsCount = points.Count;
					...
					theGraph.SetActive(points[index], pointValues[index].x > 0);
				}
			}
			// For null groups hide the appropriate lines	null 그룹의 경우 적절한 행 숨기기
			if (!neverCreateLines) {
				if (seriesIsLine) {
					if (index < 0) {
						...
						WMG_Node thePoint = points[index].GetComponent<WMG_Node>();
		...
		if (!neverCreatePoints) {
			if (hidePoints) {
				...
				if (index < 0) {
					...
					theGraph.SetActive (points [index], false);
				}
			}
		}
		if (!neverCreateLines) {
	...
	void UpdatePointWidthHeight() {
		if (!neverCreatePoints) {
			// Series line point dimensions(치수)
			if (seriesIsLine) {
				int pointsCount = points.Count;
				for (int i = 0; i < pointsCount; i++) {
					WMG_Node thePoint = points [i].GetComponent<WMG_Node> ();
					theGraph.changeSpriteHeightFloat (thePoint.objectToColor, pointWidthHeight);
					theGraph.changeSpriteWidthFloat (thePoint.objectToColor, pointWidthHeight);
				}
			}
		}
		...
		if (theGraph.legend != null) {
			WMG_Node legendPoint = legendEntry.swatchNode.GetComponent<WMG_Node> ();
			theGraph.changeSpriteHeightFloat (legendPoint.objectToColor, pointWidthHeight);
			theGraph.changeSpriteWidthFloat (legendPoint.objectToColor, pointWidthHeight);
		}
	}
	
	void UpdatePrefabType() {
		// Update prefab variable used later in the creating sprites function	스프라이트 생성 기능의 후반부에 사용된 사전 패브 변수 업데이트
		if (seriesIsLine) {
			nodePrefab = theGraph.pointPrefabs[pointPrefab];
		}
		else {
	...
	List<Vector2> sanitizeGroupData(List<Vector2> groupData) {
		if (multiSeriesEnabled && multiSeriesNumSeries > 1) return groupData;
		// Groups are defined at the graph level in the groups variable.	그룹은 그룹 변수의 그래프 수준에서 정의된다.
		// If, for example, there are 5 groups defined, then the data in the series must comprise of 5 Vector2's	예를 들어, 5개의 그룹이 정의되어 있다면, 시리즈의 데이터는 5 Vector2의 데이터로 구성되어야 한다.
		// The x value in each Vector2 represents the group, and a negative x value represents a null group.	각 Vector2의 x 값은 그룹을 나타내고, 음의 x 값은 null 그룹을 나타낸다.
		// Null groups will not be graphed at all (for example line graph with broken line segments)	Null 그룹은 그래프로 표시되지 않음(예: 끊어진 선 세그먼트가 있는 선 그래프)
		// This function will automatically group together data and insert nulls as needed.		이 기능은 자동으로 데이터를 그룹화하고 필요에 따라 null을 삽입한다.
		// For example, for 3 groups, if you supply input of (2,3) (2,5), this will convert it to (-1,0) (2,8) (-3,0)	예를 들어, 3개 그룹의 경우 (2,3)(2,5)를 입력하면 (-1,0)(-2,8)(-3,0)로 변환된다.

		// remove values that can't possibly represent groups	그룹을 나타낼 수 없는 값 제거
		for (int i = groupData.Count - 1; i >= 0; i--) {
			int intVal = Mathf.RoundToInt(groupData[i].x);
			if (intVal - groupData[i].x != 0) {
				groupData.RemoveAt(i); // Not an integer	정수가 아님
				continue;
			}
			if (Mathf.Abs(intVal) > theGraph.groups.Count) {
				groupData.RemoveAt(i); // Out of bounds		범위를 벗어남
				continue;
			}
			if (intVal == 0) {
				groupData.RemoveAt(i); // 0, because nulls are represented by negatives and there is no negative 0		0, null은 음수로 표시되고 음의 0이 없기 때문에
				continue;
			}
		}
		// sort values, combine duplicates	값 정렬, 중복 결합
		groupData.Sort( (vec1,vec2)=>vec1.x.CompareTo(vec2.x));
		List<Vector2> newPointVals = new List<Vector2>();
		bool newPoint = true;
		for (int i = 0; i < groupData.Count; i++) {
			if (newPoint) {
				newPointVals.Add(groupData[i]);
				newPoint = false;
			}
			else {
				Vector2 prev = newPointVals[newPointVals.Count-1];
				newPointVals[newPointVals.Count-1] = new Vector2(prev.x, prev.y + groupData[i].y);
			}
		...
		// insert nulls
		if (newPointVals.Count < theGraph.groups.Count) {
			int numNullsToAdd = theGraph.groups.Count - newPointVals.Count;
			for (int i = 0; i < numNullsToAdd; i++) {
				newPointVals.Insert(0, new Vector2(-1, 0));
			}
		}

		// this is rare, but there could be extras nulls (negatives), remove them until the counts are equal	이것은 드물지만, 추가 null이 있을 수 있으므로 카운트가 같을 때까지 제거하십시오.
		if (newPointVals.Count > theGraph.groups.Count) {
			int numNullsToRemove = newPointVals.Count - theGraph.groups.Count;
			for (int i = 0; i < numNullsToRemove; i++) {
				newPointVals.RemoveAt(0);
			}
		}

		// at this point, we should have for example, if 5 groups, -1, -1, 1, 2, 5		이 시점에서 예를 들어 5개 그룹, -1, -1, 1, 2, 5가 있어야 한다.
		// now to easily determine which groups are null, need something like 1, 2, -3, -4, 5		이제 어떤 그룹이 null인지 쉽게 판단하려면 1, 2, -3, -4, 5와 같은 것이 필요하다.
		List<int> nullGroups = new List<int>();
		for (int i = 0; i < theGraph.groups.Count; i++) {
			nullGroups.Add(i+1);
		}
		for (int i = newPointVals.Count - 1; i >= 0; i--) {
			if (newPointVals[i].x > 0) nullGroups.Remove(Mathf.RoundToInt(newPointVals[i].x));
		}
		for (int i = 0; i < nullGroups.Count; i++) {
			newPointVals[i] = new Vector2(-1*nullGroups[i], 0);
		}

		// sort values, so that negatives treated same as positives		값을 정렬하여 음수가 양수와 동일하게 처리되도록 함
		newPointVals.Sort( (vec1,vec2)=>Mathf.Abs(vec1.x).CompareTo(Mathf.Abs(vec2.x)));

		return newPointVals;
	}

	void createOrDeletePoints(int pointValuesCount) {
		// Create points based on pointValues data		점 기준 점 생성값 데이터
		int pointsCount = points.Count;
		for (int i = pointsCount; i < pointValuesCount; i++) {
			if (!neverCreatePoints) {
				...
				curNode.radius = -1 * linePadding;
				curNode.seriesRef = this;
				...
				if (!neverCreateLines) {
					if (i > 0) {
						...
						lines.Add(curObj);
						OnLineCreated(this, curObj, i-1);
					}
				}
				barIsNegative.Add(false);
			}
		}
		// If there are more points than pointValues data, delete the extras	점 값 데이터보다 점이 많으면 추가 값을 삭제하십시오.
		...
		for (int i = pointsCount - 1; i >= 0; i--) {
			...
			// Delete existing connect first to last	기존 연결의 첫번째부터 마지막까지 삭제
			if (!neverCreateLines) {
		...
		// Create line connecting first and last points		첫 번째 점과 마지막 점을 연결하는 선 작성
		if (!neverCreateLines) {
			if (points.Count > 2) {
				WMG_Node firstNode = points[0].GetComponent<WMG_Node>();
				...
				if (connectFirstToLast && delLink == null) {
					...
					lines.Add(curObj);
					OnLineCreated(this, curObj, lines.Count-1);
				}
				if (!connectFirstToLast && delLink != null) {
					lines.Remove(delLink.gameObject);
					theGraph.DeleteLink(delLink);
		...
		// Create the legend if it doesn't exist (changing prefab type deletes the legend swatch and lines)		범례가 없는 경우 범례 만들기(사전 패브 유형을 변경하면 범례 스왓치와 선이 삭제됨)
		if (theGraph.legend != null) {
			if (legendEntry.swatchNode == null) {
				createLegendSwatch ();
			}
	...
	void createOrDeleteLabels(int pointValuesCount) {
		...
		if (dataLabelPrefab != null && dataLabelsParent != null) {
			...
			if (!dataLabelsEnabled) {
				...
				// Data labels doesn't work for stacked bar or stacked percentage bar	쌓인 막대 또는 쌓인 백분율 막대에는 데이터 레이블이 작동하지 않음
				if (theGraph.IsStacked && theGraph.graphType != WMG_Axis_Graph.graphTypes.line_stacked) {
					numLabels = 0;
					dataLabelsEnabled = false;
				}
			}
			// If there are more data labels than pointValues data, delete the extras	pointValues 데이터보다 데이터 레이블이 더 많은 경우 추가 데이터 삭제
			...
			if (!areaShadingUsesComputeShader) StartCoroutine(SetDelayedAreaShadingChanged ()); // For some reason creating / deleting objects hides area shading	어떤 이유로 개체를 만들거나 삭제하면 영역 음영 처리
		}
	}

	void createOrDeleteAreaShading(int pointValuesCount) {
		if (areaShadingUsesComputeShader || linesUseComputeShader) {
			if (!WMG_Util.CanUseComputeShader()) {
				linesUseComputeShader = false;
				areaShadingUsesComputeShader = false;
			}
			...
			if (areaShadingType != areaShadingTypes.None && areaShadingRects.Count == 1 && areaShadingRects[0].name == "Area_Shading_CS") {
				UpdateSprites(); // changed from gradient to fill or vice versa		경사도를 변경
				return;
			}
			...
			if (areaShadingType != areaShadingTypes.None || linesUseComputeShader) {
				if (areaShadingRects.Count != 1) {
					...
					theGraph.setAnchor(curObj, new Vector2(0.5f, 0.5f), Vector2.zero, Vector2.zero);
			...
			// Create area shading rectangles based on pointValues data		점을 기준으로 영역 음영 사각형 만들기값 데이터
			...
			// If there are more shading rectangles than pointValues data, delete the extras	점 값 데이터보다 음영 사각형이 더 많은 경우 추가 값을 삭제하십시오.
			for (int i = areaShadingRects.Count - 1; i >= 0; i--) {
	...
	void UpdateSprites(int index = -1) {
		if (points.Count == 0) {
			if ((areaShadingUsesComputeShader || linesUseComputeShader) && areaShadingRects.Count == 1) {
				theGraph.SetActive (areaShadingRects [0], false);
			}
			if (!neverCreatePoints) {
				return;
			}
		...
		// if beginning auto animation, set the animation starting positions	자동 애니메이션을 시작할 경우 애니메이션 시작 위치 설정
		if (beginningToAutoAnimate) { 
			if (!currentlyAnimating) { // not already in the middle of another animation
				...
				// set new original positions based on previous animation timeline that was just now interrupted	방금 중단되었던 이전 애니메이션 시간 표시 막대에 따라 새로운 원래 위치 설정
				int afterPositionsCount = afterPositions.Count;
				List<Vector2> newPositions = new List<Vector2>(afterPositionsCount); 
				List<int> newWidths = new List<int>(afterPositionsCount); 
				List<int> newHeights = new List<int>(afterPositionsCount);
				for (int i = 0; i < afterPositionsCount; i++) {
		...
		if (beginningToAutoAnimate)
		{ // if beginning auto animation, trigger the animation to begin	자동 애니메이션을 시작하는 경우 시작할 애니메이션 트리거
		  // update new positions / animation end positions		새 위치/애니메이션 종료 위치 업데이트
			getNewPointPositionsAndSizes(prevPoints, -1); // single point animation tricky, because animation can be interrupted by another animation in a later frame, and the position lists would not be correct		싱글 포인트 애니메이션은 이후 프레임의 다른 애니메이션에 의해 중단될 수 있고 위치 목록이 정확하지 않기 때문에 까다롭다.
			OnSeriesAutoAnimStarted ();
			beginningToAutoAnimate = false;
		} else {
			// update new positions / animation end positions	새 위치/애니메이션 종료 위치 업데이트
			getNewPointPositionsAndSizes(prevPoints, index);
			UpdateVisuals (index);
		}
	}

	public void UpdateVisuals(int index = -1, List<Vector2> newPositions = null, List<int> newWidths = null, List<int> newHeights = null) {
		updatePointSprites(index, newPositions, newWidths, newHeights);
		updateDataLabels();
		updateAreaShading();
		int pointsCount = points.Count;
		for (int i = 0; i < pointsCount; i++) {
	...
	public void updateXdistBetween() {
		// Auto set xDistBetween based on the axis length and point count	축 길이 및 점 개수를 기준으로 xDistBetween 자동 설정
		if (!ManuallySetXDistBetween) {
			_xDistBetweenPoints = theGraph.getDistBetween(neverCreatePoints ? numPointsPerSeries : points.Count, 
			                                              (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal ? theGraph.yAxisLength : theGraph.xAxisLength));
		}
	}

	public void updateExtraXSpace() {
		// auto update space from axis		축에서 공간 자동 업데이트
		if (!ManuallySetExtraXSpace) {
			if (theGraph.autoUpdateSeriesAxisSpacing) {
				if (theGraph.graphType == WMG_Axis_Graph.graphTypes.line || theGraph.graphType == WMG_Axis_Graph.graphTypes.line_stacked) {
					...
					int lineSeriesCount = theGraph.lineSeries.Count;
	...
	void getNewPointPositionsAndSizes(List<GameObject> prevPoints, int index) {
		updateXdistBetween();
		
		updateExtraXSpace();

		// cache various values before looping through points - performance optimization	여러 값을 캐싱한 후 점을 반복 - 성능 최적화
		int pointValuesCount = pointValues.Count;
		int pointWidthHeightInt = Mathf.RoundToInt(pointWidthHeight);
		int theGraphBarWidthInt = Mathf.RoundToInt (theGraph.barWidth);
		float theGraphyAxisLengthOrienInd = theGraph.yAxisLengthOrienInd;
		float theGraphxAxisLengthOrienInd = theGraph.xAxisLengthOrienInd;
		float yAxisOrienIndAxisMinValue = yAxisOrienInd.AxisMinValue;
		float xAxisOrienIndAxisMinValue = xAxisOrienInd.AxisMinValue;
		float yAxisOrienIndAxisMaxValue = yAxisOrienInd.AxisMaxValue;
		float xAxisOrienIndAxisMaxValue = xAxisOrienInd.AxisMaxValue;
		float theGraphBarWidth = theGraph.barWidth;
		bool isVertical = theGraph.orientationType == WMG_Axis_Graph.orientationTypes.vertical;
		bool isLineStacked = theGraph.graphType == WMG_Axis_Graph.graphTypes.line_stacked;
		bool _seriesIsLine = seriesIsLine;
		bool theGraphUseGroups = theGraph.useGroups;

		int afterPositionsCount = afterPositions.Count;
		if (afterPositionsCount < pointValuesCount) {
			for (int i = afterPositionsCount; i < pointValuesCount; i++) {
				afterWidths.Add(0);
				afterHeights.Add(0);
				afterPositions.Add(Vector2.zero);
			}
		}
		if (afterPositionsCount > pointValuesCount) {
			int numElementsToRemove = afterPositionsCount - pointValuesCount;
			afterWidths.RemoveRange(pointValuesCount, numElementsToRemove);
			afterHeights.RemoveRange(pointValuesCount, numElementsToRemove);
			afterPositions.RemoveRange(pointValuesCount, numElementsToRemove);
		}
		...
		for (int i = 0; i < pointValuesCount; i++) {
			...
			float newX = 0;
			float newY = (pointValues[i].y - yAxisOrienIndAxisMinValue)/(yAxisOrienIndAxisMaxValue - yAxisOrienIndAxisMinValue) * theGraphyAxisLengthOrienInd; // new y always based on the pointValues.y

			// If using xDistBetween then point positioning based on previous point point position		xDistBetween을 사용하는 경우 이전 점 위치를 기준으로 점 위치 지정
			if (!theGraphUseGroups && _UseXDistBetweenToSpace) {
				if (i > 0)
				{ // For points greater than 0, use the previous point position plus xDistBetween	0보다 큰 점의 경우 이전 점 위치 + xDistBetween을 사용하십시오.
					float prevPosX = (isVertical) ? afterPositions[i-1].x : afterPositions[i-1].y;
					newX = prevPosX + xDistBetweenPoints;
				}
				else
				{ // For point 0, set initial x position to extraXSpace		점 0의 경우 초기 x 위치를 extraXSpace로 설정
					newX = extraXSpace - (!_seriesIsLine && !isVertical ? theGraphBarWidth : 0);
				}
			}
			else if (theGraphUseGroups) { // Using groups, x values represent integer index of group	그룹을 사용하여 x 값은 그룹의 정수 색인을 나타냄
				newX = extraXSpace + xDistBetweenPoints * (Mathf.Abs(pointValues[i].x) - 1);
				newX -= (!_seriesIsLine && !isVertical ? theGraphBarWidth : 0);
			}
			else
			{ // Not using xDistBetween or groups, so use the actual x values in the Vector2 list		xDistBetween 또는 그룹을 사용하지 않으므로 Vector2 목록의 실제 x 값을 사용하십시오.
				newX = (pointValues[i].x - xAxisOrienIndAxisMinValue)/(xAxisOrienIndAxisMaxValue - xAxisOrienIndAxisMinValue) * theGraphxAxisLengthOrienInd;
			}
			...
			if (_seriesIsLine) {
				newWidth = pointWidthHeightInt;
				newHeight = pointWidthHeightInt;
				
				if (isLineStacked) {
					if (prevPoints != null && i < prevPoints.Count) {
						newY += (isVertical) ? theGraph.getSpritePositionY(prevPoints[i]) 
				...
				if (!neverCreatePoints) {
					// For bar graphs, need to update sprites width and height based on positions		막대 그래프의 경우 위치에 따라 스프라이트 너비와 높이를 업데이트해야 함
					// For stacked percentage, need to set a y position based on the percentage of all series values combined		쌓인 백분율의 경우 결합된 모든 영상 시리즈 값의 백분율을 기준으로 y 위치를 설정해야 함
					if (theGraph.graphType == WMG_Axis_Graph.graphTypes.bar_stacked_percent && theGraph.TotalPointValues.Count > i) {
						newY = (pointValues[i].y - yAxisOrienIndAxisMinValue) / theGraph.TotalPointValues[i] * theGraphyAxisLengthOrienInd;
					}

					int barAxisAdjust = 0; // adjustment based on bar axis value	막대 축 값에 기반한 조정
					if (theGraph.graphType == WMG_Axis_Graph.graphTypes.bar_side || (theGraph.graphType == WMG_Axis_Graph.graphTypes.combo && comboType == comboTypes.bar)) {
						barAxisAdjust = Mathf.RoundToInt((theGraph.barAxisValue - yAxisOrienIndAxisMinValue) / (yAxisOrienIndAxisMaxValue - yAxisOrienIndAxisMinValue) * theGraphyAxisLengthOrienInd);
					}
					...
					// Update sprite dimensions and increase position using previous point position		이전 포인트 위치를 사용하여 스프라이트 치수를 업데이트하고 위치를 늘리십시오.
					newWidth = theGraphBarWidthInt;
					newHeight = Mathf.RoundToInt(newHeightOrienInd);

					// Previous points is null for side by side bar, but should not be empty for stacked and stacked percentage for series after the first series		이전 점은 나란히 있는 막대에 대해 null이지만 첫 번째 시리즈 이후 시리즈에 대해 쌓인 비율과 쌓인 비율에 대해서는 비워 둘 수 없음
					if (prevPoints != null && i < prevPoints.Count) {
						newY += (isVertical) 
			...
			afterWidths[i] = newWidth;
			afterHeights[i] = newHeight;
			afterPositions[i] = new Vector2(newX, newY);
		}
	}
	
	void updatePointSprites(int index = -1, List<Vector2> newPositions = null, List<int> newWidths = null, List<int> newHeights = null) {
		if (!neverCreatePoints) {
			int pointValuesCount = pointValues.Count;
			bool _seriesIsLine = seriesIsLine;
			for (int i = 0; i < pointValuesCount; i++) {
	...
	void updateDataLabels() {
		...
		int pointsCount = neverCreatePoints ? pointValues.Count : points.Count;
		int dataLabelsCount = dataLabels.Count;
		for (int i = 0; i < pointsCount; i++) {
			if (i >= dataLabelsCount) break;
			updateADataLabel(i);
		}
	}
	...
	void updateADataLabel(int i) {
		if (!dataLabelsEnabled) return;
		Vector2 currentPointPosition = new Vector2(neverCreatePoints ? afterPositions[i].x : theGraph.getSpritePositionX(points[i]), 
		                                           neverCreatePoints ? afterPositions[i].y : theGraph.getSpritePositionY(points[i]));
		// Update font size
		...
		// Update text based on y value and number decimals		y 값 및 숫자 십진수에 따라 텍스트 업데이트
	...
	// Update the position, alpha clipping, and other properties of the area shading rectangles		영역 음영 사각형의 위치, 알파 클리핑 및 기타 속성 업데이트
	void updateAreaShading() {
		if (areaShadingType == areaShadingTypes.None && !linesUseComputeShader) return;
		if ((areaShadingUsesComputeShader || linesUseComputeShader) && areaShadingRects.Count == 1)
		{ // all values (min, max, and the points) represent a percentage of graph		모든 값(최소, 최대 및 점)은 그래프의 백분율을 나타냄
			int pointValuesCount = pointValues.Count;
			if (pointValuesCount < 2) { // need at least 2 points to draw anything		무엇이든 그리려면 적어도 2개의 점이 필요하다.
				theGraph.SetActive(areaShadingRects[0], false);
				return; 
			} else {
				theGraph.SetActive(areaShadingRects[0], true);
			}
			...
			areaShadingCSdata.setupPixelIndexMap(areaShadingTextureResolution);

			float minGraphSize = Mathf.Min(theGraph.getSpriteWidth(theGraph.seriesParent.gameObject), theGraph.getSpriteHeight(theGraph.seriesParent.gameObject));
			float halfLineWidth = lineScale * 2f * ((float)areaShadingTextureResolution / minGraphSize);
			...
			areaShadingCS.computeShader.SetFloats ("lineColor", new float[]{ lineColor.r, lineColor.g, lineColor.b, lineColor.a });
			areaShadingCS.computeShader.SetInt("numPointsPerSeries", numPointsPerSeries);
			areaShadingCS.computeShader.SetInt("numSeries", multiSeriesEnabled ? multiSeriesNumSeries : 1);
			areaShadingCS.computeShader.SetInt("isAreaShading", areaShadingType != areaShadingTypes.None ? 1 : 0);
			areaShadingCS.computeShader.SetInt("isFill", areaShadingType == areaShadingTypes.Solid ? 1 : 0);
			areaShadingCS.computeShader.SetInt("drawLines", linesUseComputeShader ? 1 : 0);
			areaShadingCS.computeShader.SetFloat ("halfLineWidth", halfLineWidth);
			areaShadingCS.computeShader.SetInt("isHorizontal", theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal ? 1 : 0);
			if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal) {
				areaShadingCS.computeShader.SetFloat ("shadingAxisVal", (areaShadingAxisValue - theGraph.xAxis.AxisMinValue) / (theGraph.xAxis.AxisMaxValue - theGraph.xAxis.AxisMinValue));
			}
			else {
				areaShadingCS.computeShader.SetFloat ("shadingAxisVal", (areaShadingAxisValue - yAxis.AxisMinValue) / (yAxis.AxisMaxValue - yAxis.AxisMinValue));
			}
			float maxVal = 0; // as a percentage of axis	축의 비율로
			float minVal = 1; // as a percentage of axis

			areaShadingCSdata.EnsureCapacity(pointValuesCount);
			float theGraphxAxisLength = theGraph.xAxisLength;
			float theGraphyAxisLength = theGraph.yAxisLength;
			bool isHorizontal = theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal;
			bool graphUsesGroups = theGraph.useGroups;
			int previousPixelIndex = 0;

			int halfLineWidthNumPixels = Mathf.RoundToInt(halfLineWidth);
			for (int i = 0; i < pointValuesCount; i++) {
				if (neverCreatePoints) { // automatic animations dont work here		여기서는 자동 애니메이션이 작동하지 않음
					areaShadingCSdata.xVals[i] = isHorizontal ? (afterPositions[i].y / theGraphyAxisLength) : (afterPositions[i].x / theGraphxAxisLength);
					areaShadingCSdata.yVals[i] = isHorizontal ? (afterPositions[i].x / theGraphxAxisLength) : (afterPositions[i].y / theGraphyAxisLength);
				} else {
					areaShadingCSdata.xVals[i] = isHorizontal ? (points[i].transform.localPosition.y / theGraphyAxisLength) : (points[i].transform.localPosition.x / theGraphxAxisLength);
					areaShadingCSdata.yVals[i] = isHorizontal ? (points[i].transform.localPosition.x / theGraphxAxisLength) : (points[i].transform.localPosition.y / theGraphyAxisLength); 
				}

				if (areaShadingCSdata.yVals[i] > maxVal) {
					maxVal = areaShadingCSdata.yVals[i];
				}
				if (areaShadingCSdata.yVals[i] < minVal) {
					minVal = areaShadingCSdata.yVals[i];
				}

				if (i < numPointsPerSeries) {
					int newPixelIndex = Mathf.Clamp(Mathf.RoundToInt(areaShadingCSdata.xVals[i] * areaShadingTextureResolution), 0, areaShadingTextureResolution);
					if (i == 0) {
						newPixelIndex = Mathf.Max(0, newPixelIndex - halfLineWidthNumPixels);
					}
					if (i == numPointsPerSeries-1) {
						newPixelIndex = Mathf.Min(areaShadingTextureResolution, newPixelIndex + halfLineWidthNumPixels);
					}
					for (int j = previousPixelIndex; j < newPixelIndex; j++) {
						areaShadingCSdata.pixelIndexToPointMap[j] = (uint)i;
					}
					previousPixelIndex = newPixelIndex;
				}

				if (graphUsesGroups && pointValues[i].x < 0) { // null data point
					areaShadingCSdata.xVals[i] = float.NaN;
					areaShadingCSdata.yVals[i] = float.NaN;
				}
				if (!graphUsesGroups && float.IsNaN(pointValues[i].y)) {// null data point
					areaShadingCSdata.xVals[i] = float.NaN;
					areaShadingCSdata.yVals[i] = float.NaN;
				}
			}
			...
			areaShadingCS.computeShader.SetFloat ("minVal", minVal);

			areaShadingCSdata.pointValsBufferX.SetData (areaShadingCSdata.xVals);
			areaShadingCSdata.pointValsBufferY.SetData (areaShadingCSdata.yVals);
			areaShadingCSdata.pixelIndexToPointMapBuffer.SetData (areaShadingCSdata.pixelIndexToPointMap);

			areaShadingCS.computeShader.SetBuffer (areaShadingCS.getKernelHandle(), "xVals", areaShadingCSdata.pointValsBufferX);
			areaShadingCS.computeShader.SetBuffer (areaShadingCS.getKernelHandle(), "yVals", areaShadingCSdata.pointValsBufferY);
			areaShadingCS.computeShader.SetBuffer (areaShadingCS.getKernelHandle(), "pixelIndexToPointIndexMap", areaShadingCSdata.pixelIndexToPointMapBuffer);

			if (multiSeriesEnabled && multiSeriesNumSeries > 1 && multiSeriesTextureLineColors != null && multiSeriesTextureLineColors.Count > 1) {
				areaShadingCS.computeShader.SetInt("useMultiColors", 1);
				areaShadingCSdata.setupMultiSeriesColors (multiSeriesNumSeries);
				for (int i = 0; i < multiSeriesTextureLineColors.Count; i++) {
					areaShadingCSdata.multiSeriesLineColors [i * 4 + 0] = multiSeriesTextureLineColors [i].r;
					areaShadingCSdata.multiSeriesLineColors [i * 4 + 1] = multiSeriesTextureLineColors [i].g;
					areaShadingCSdata.multiSeriesLineColors [i * 4 + 2] = multiSeriesTextureLineColors [i].b;
					areaShadingCSdata.multiSeriesLineColors [i * 4 + 3] = multiSeriesTextureLineColors [i].a;
				}
				areaShadingCSdata.multiSeriesLineColorsBuffer.SetData (areaShadingCSdata.multiSeriesLineColors);
				areaShadingCS.computeShader.SetBuffer (areaShadingCS.getKernelHandle (), "multiSeriesLineColors", areaShadingCSdata.multiSeriesLineColorsBuffer);
			} else {
				areaShadingCS.computeShader.SetInt("useMultiColors", 0);
			}
			...
			// Find the maximum area shading height so that we can corectly adjust each sprites transparency based on their height in comparison to the max height		최대 높이와 비교하여 각 스프라이트의 높이에 따라 투명도를 핵심적으로 조정할 수 있도록 최대 면적 음영 높이를 찾으십시오.
			float maxVal = Mathf.NegativeInfinity;
			int pointsCount = points.Count;
			int pointValuesCount = pointValues.Count;
			for (int i = 0; i < pointsCount; i++) {
				if (i >= pointValuesCount) break;
			...
			for (int i = 0; i < pointsCount - 1; i++) {
				...
				// Adjust previous sprite width based on previous width and position so that the sprites do not overlap due to position being a float and width being an integer	이전 너비와 위치를 기준으로 이전 스프라이트 너비를 조정하여 스프라이트가 플로트 위치, 너비가 정수인 관계로 겹치지 않도록 하십시오.
				if (i > 0) { // Don't need to adjust the width for the first rectangle		첫 번째 직사각형의 너비를 조정할 필요가 없음
					if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal) {
						...
						// if y previous < y current - width current then increase current width by 1y 1	y 이전 < y 현재 - 현재 폭 인 경우, 현재 폭을 1y 1만큼 증가시킨다.
						...
						// if y previous > y current - width current then decrease current width by 1y 1	y 이전 > y 현재 - 현재 폭 인 경우 현재 폭을 1y 1만큼 감소시킨다.
						if (previousSpriteWidthPlusPosition < yPrevious) {
							theGraph.changeSpriteWidth(areaShadingRects[i], Mathf.RoundToInt(theGraph.getSpriteWidth(areaShadingRects[i]) - 1));
						}
					}
						...
						// If greater then sprites would overlap, subtract width by 1	스프라이트가 커 스프라이트가 겹칠 경우 너비를 1만큼 빼십시오.
						...
						// If lesser then sprites would have gap, increased width by 1	스프라이트가 작아 간극이 생길 경우 폭을 1 증가
						if (previousSpriteWidthPlusPosition < Mathf.RoundToInt(theGraph.getSpritePositionX(areaShadingRects[i]))) {
							theGraph.changeSpriteWidth(areaShadingRects[i-1], Mathf.RoundToInt(theGraph.getSpriteWidth(areaShadingRects[i-1]) + 1));
						}
					}
				...
				// Set custom shader properties to do appropriate alpha clipping, gradient shading, color, etc.		적절한 알파 클리핑, 그라데이션 쉐이딩, 색상 등을 수행하도록 사용자 지정 셰이더 속성을 설정하십시오.
				...
				// Set the gradient scale based on current sprite height in comparison to maximum sprite height		최대 스프라이트 높이에 비해 현재 스프라이트 높이를 기준으로 그라데이션 눈금 설정
	...
	/// <summary>
	/// When using texture based line drawing (#linesUseComputeShader = true) and drawing many series (#multiSeriesEnabled = true and #multiSeriesNumSeries > 1), then you this to control the color of the line colors.
	/// 텍스처 기반 선 그리기(#lineUseComputeShader = true)를 사용하고 많은 시리즈(#multiSeriesEnabled = true 및 #multiSeriesNumSeries > 1)를 그릴 때 선 색상을 제어하려면 이 옵션을 사용하십시오.
	/// </summary>
	public void SetMultiSeriesTextureLineColors(List<Color> lineColors) {
		multiSeriesTextureLineColors = new List<Color>(lineColors);
		areaShadingC.Changed ();
	}

	/// <summary>
	/// When using realtime updating (#realTimeDataSource != null), begins the real time updating.
	/// 실시간 업데이트(#realTimeDataSource != null)를 사용하면 실시간 업데이트를 시작한다.
	/// </summary>
	...
	/// 실시간 업데이트(#realTimeDataSource != null)를 사용할 경우 이전에 #StartRealTimeUpdate로 호출한 후 데이터 업데이트를 일시 중지하거나 중지하십시오.
	/// </summary>
	...
	/// 실시간 업데이트(#RealTimeDataSource != null)를 사용하는 경우 이전에 #StopRealTimeUpdate를 호출한 후 일시 중지된 상태에서 실시간 업데이트를 재개하십시오.
	/// </summary>
	...
	void DoRealTimeUpdate() {
		/* This "Real Time" update is FPS dependent, so the time axis actually represents a number of frames.	이 "실시간" 업데이트는 FPS에 의존하므로, 시간축은 실제로 여러 프레임을 나타낸다.
		 * The waitForSeconds for coroutines does not actually wait for the specified number of seconds, and is also FPS dependent.		코루틴의 waitForSeconds는 실제로 지정된 시간(초)을 기다리지 않으며 FPS에 의존하기도 한다.
		 * An FPS independent solution only seems possible with fixedUpdate, which may be added later.	FPS 독립 솔루션은 나중에 추가할 수 있는 fixedUpdate에서만 가능한 것으로 보인다. */

		float waitTime = 0.0166f; // Each x-axis unit is 60 frames. This is 1 second at 60 fps.		각 x축 단위는 60프레임이다. 이것은 60fps에서 1초다.
		...
		// Add new point or move the last existing point	새 점 추가 또는 마지막 기존 점 이동
		if (pointValues.Count >= 2) {
			float maxStdDev = 0.3f; // a standard deviation above this means generate a new point	이 평균 이상의 표준 편차는 새로운 점을 생성한다.
			// use graph slope to normalize the calculated slopes.	그래프 기울기를 사용하여 계산된 기울기를 정규화하십시오.
			...
			// For 2 points this is equivalent to the standard deviation	두 점의 경우 이는 표준 편차와 동일하다.
			if (Mathf.Abs(slopes[0]-slopes[1]) <= maxStdDev) { // if standard deviation less than threshold, then move point	표준 편차가 임계값보다 작은 경우 점 이동
				// Slopes about the same, move the last point	기울기가 거의 같음, 마지막 점 이동
				pointValues[pointValues.Count-1] = new Vector2(realTimeLoopVar,yval);
			}
			else {
				// Slopes significantly different, add a new point	기울기가 유의하게 다르므로 새 점을 추가하십시오.
				pointValues.Add(new Vector2(realTimeLoopVar, yval));
			}
		}
		else {
			// Just add the second point	두 번째 점을 추가하십시오.
			pointValues.Add(new Vector2(realTimeLoopVar, yval));
		}

		// If needed, change graph axis boundary and remove or move the first point to keep the series within the graph boundaries		필요한 경우 그래프 축 경계를 변경하고 첫 번째 점을 제거하거나 이동하여 시리즈를 그래프 경계 내에 유지하십시오.
		if (pointValues.Count > 1 && pointValues[pointValues.Count-1].x > realTimeOrigMax) {

			// For the last real time update series update the axis boundaries by the difference		마지막 실시간 업데이트 시리즈에 대해 차이에 따라 축 경계 업데이트
			if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.vertical) {
			...
			// First and second points used to see if the first point should be moved or deleted after incrementing the minimum axis value		최소 축 값을 증가시킨 후 첫 번째 점을 이동 또는 삭제할지 여부를 확인하는 데 사용되는 첫 번째 및 두 번째 점
			float x1 = pointValues[0].x;
			float x2 = pointValues[1].x;
			float y1 = pointValues[0].y;
			float y2 = pointValues[1].y;

			// Delete or move the very first point to keep the series in the graph boundary when the maximum is increased		최대값이 증가할 때 시리즈를 그래프 경계로 유지하기 위해 첫 번째 점 삭제 또는 이동
	...
	/// 시리즈를 삭제할 때 내부 도우미 기능은 Graph Maker 코드 외부로 호출해서는 안 된다.
	/// </summary>
